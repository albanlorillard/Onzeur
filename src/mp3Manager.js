...
var perserFolder = fs.readdir('./dossiermp3', function(err, items) {
      ...
      for (var i=0; i<items.length; i++) {
        ...
        var parser = mm(readableStream, function (err, metadata) {
          ...
          if (err) throw err;
          var laPoule= db.query("insert into listmp3 (title, artist,genre,url) values ('"+metadata.title.replace(/'/g,"\\'")+"', '"+metadata.artist[0].replace(/'/g,"\\'")+"','"+metadata.genre[0].replace(/'/g,"\\'")+"','./dossiermp3/"+items[glob]+"');");
...
/*
CREATE TABLE `musiconzeur`.`listmp3` (
